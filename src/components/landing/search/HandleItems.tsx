...
interface HandleItemsProps {
  items: SearchType[];
  setItems: (value: (prevItems: SearchType[]) => SearchType[]) => void;
  PostItem: (name: string, classes: string[]) => Promise<SearchType>;
  DeleteItem: (id: string) => void;
  PatchItemId: (item: SearchType) => Promise<SearchType>;
  PatchItems: (items: SearchType[]) => Promise<SearchType[]>;
...
export default function HandleItems({
  ...
  const [itemToEdit, setItemToEdit] = useState<SearchType | null>(null);
  const [editDnD, setEditDnD] = useState(false);

  async function addNewItem(newItem: SearchType) {
  ...
  // ---------- EDIT ITEM ---------- //
  async function editItem(updatedItem: SearchType) {
  ...
  return (
    <>
      <Card className={`${jost.className} w-full md:max-w-2xl`}>
